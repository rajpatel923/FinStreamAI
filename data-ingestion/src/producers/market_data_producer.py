...
class MarketDataProducer(BaseProducer):
    ...
    def __init__(self) -> None:
        ...
        self._use_mock = cfg.USE_MOCK_DATA or (not self._polygon_key and not self._av_key)
        ...
        source = "mock" if self._use_mock else ("polygon" if self._polygon_key else "alpha_vantage")
        logger.info("MarketDataProducer initialised", source=source, symbols=self._symbols)

    def data_source_name(self) -> str:
        if self._use_mock:
            return "mock"
        return "polygon" if self._polygon_key else "alpha_vantage"

    def poll_interval_seconds(self) -> float:
        if self._use_mock:
            return 1.0
        return 1.0 if self._polygon_key else 12.0
